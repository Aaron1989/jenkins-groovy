...
def COMPILE_JOB_NAME = Folder+'/'+'Stg-Qypay-MS-QuyiVersionManage-Compile' //编译job名称，不可以使用中文
def DEPLOY_JOB_NAME  = Folder+'/'+'Stg-Qypay-MS-QuyiVersionManage-Deploy' //发布job名称，不可以使用中文
def PIPELINE_JOB_NAME1 = Folder+'/'+'Stg-Qypay-MS-QuyiVersionManage-Pipeline'

def GITURL = 'http://h.quyiyuan.com/scm/qypay/ms-quyiversionmanage.git' //git仓库地址
...
def GROUP = 'qypay'  //所属组
def PRODUCTION = 'ms-quyiversionmanage'   //产品名称，该参数表示oss存储路径和部署服务器路径中微服务名
def WAR_NAME = 'ms-quyiversionmanage.war'  //WAR包名称
...
pipelineJob("$PIPELINE_JOB_NAME1") {
    ...
    definition {
        cpsScm {
            ...
            scriptPath("jenkins-pipelines/staging/Qypay/Stg-Qypay-MS-QuyiVersionManage-Pipeline.groovy")
        }
    }
}