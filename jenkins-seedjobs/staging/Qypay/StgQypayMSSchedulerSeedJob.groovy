...
def COMPILE_JOB_NAME = Folder+'/'+'Stg-Qypay-MS-Scheduler-Compile' //编译job名称，不可以使用中文
def DEPLOY_JOB_NAME = Folder+'/'+'Stg-Qypay-MS-Scheduler-Deploy' //发布job名称，不可以使用中文
def PIPELINE_JOB_NAME1 = Folder+'/'+'Stg-Qypay-MS-Scheduler-Pipeline'
...
pipelineJob("$PIPELINE_JOB_NAME1") {
    ...
    definition {
        cpsScm {
            ...
            scriptPath("jenkins-pipelines/staging/Qypay/Stg-Qypay-MS-Scheduler-Pipeline.groovy")
        }
    }
}