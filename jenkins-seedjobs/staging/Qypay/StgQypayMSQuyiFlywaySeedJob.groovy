...
def COMPILE_JOB_NAME = Folder+'/'+'Stg-Qypay-MS-QuyiFlyway-Compile' //编译job名称，不可以使用中文
def DEPLOY_JOB_NAME  = Folder+'/'+'Stg-Qypay-MS-QuyiFlyway-Deploy' //发布job名称，不可以使用中文
def PIPELINE_JOB_NAME1 = Folder+'/'+'Stg-Qypay-MS-QuyiFlyway-Pipeline'
...
def GROUP = 'qypay'  //所属组
def PRODUCTION = 'ms-quyiflyway'   //产品名称，该参数表示oss存储路径和部署服务器路径中微服务名
def WAR_NAME = 'ms-quyiflyway.war'  //WAR包名称
...
pipelineJob("$PIPELINE_JOB_NAME1") {
    ...
    definition {
        cpsScm {
            ...
            scriptPath("jenkins-pipelines/staging/Qypay/Stg-Qypay-MS-QuyiFlyway-Pipeline.groovy")
        }
    }
}