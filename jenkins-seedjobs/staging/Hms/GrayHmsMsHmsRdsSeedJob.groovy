...
def COMPILE_JOB_NAME = Folder+'/'+'Gray-Hms-MSHMS-RDS-Compile' //编译job名称，不可以使用中文
def DEPLOY_JOB_NAME = Folder+'/'+'Gray-Hms-MSHMS-RDS-Deploy' //发布job名称，不可以使用中文
def PIPELINE_JOB_NAME =  Folder+'/'+'Gray-Hms-MSHMS-RDS-Pipeline'
def ECS_IP = [' ']

def GITURL = 'http://h.quyiyuan.com/scm/ysapp/hms-rds.git' //git仓库地址
...
def ENV = 'staging'
def GROUP = 'hms'  //所属组
def PRODUCTION = 'ms-hms-rds'   //产品名称，该参数表示oss存储路径和部署服务器路径中微服务名
def WAR_NAME = 'hms-rds-1.0.0.jar'  //WAR包名称

def EXPORT =''
def APP_NAME = 'ms-hms-rds'  //微服务名称,一般固定在代码里，若不修改则为空
...
pipelineJob("$PIPELINE_JOB_NAME") {
    ...
    definition {
        cpsScm {
            ...
            scriptPath("jenkins-pipelines/staging/Hms/Gray-Hms-MsHms-Rds-Pipeline.groovy")
        }
    }
}