...
def COMPILE_JOB_NAME = Folder+'/'+'Gray-Hms-MS-Telemed-Compile' //编译job名称，不可以使用中文
def DEPLOY_JOB_NAME = Folder+'/'+'Gray-Hms-MS-Telemed-Deploy' //发布job名称，不可以使用中文
def PIPELINE_JOB_NAME =  Folder+'/'+'Gray-Hms-MS-Telemed-Pipeline'
def ECS_IP = [ ' ']
...
def WAR_NAME = 'MS-TeleMed-*'  //WAR包名称

def EXPORT =''
def APP_NAME = 'ms-telemed-pre'  //微服务名称,一般固定在代码里，若不修改则为空
...
pipelineJob("$PIPELINE_JOB_NAME") {
    ...
    definition {
        cpsScm {
            ...
            scriptPath("jenkins-pipelines/staging/Hms/Gray-Hms-MsTelemed-Pipeline.groovy")
        }
    }
}
