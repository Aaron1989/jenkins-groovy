...
def COMPILE_JOB_NAME = Folder+'/'+'Prod-Plugins-MsWebcomponent-Compile' //编译job名称，不可以使用中文
def DEPLOY_JOB_NAME1 = Folder+'/'+'Prod-Plugins-MsWebcomponent-Deploy1' //发布job名称，不可以使用中文
...
def PRODUCTION = 'ms-webcomponent'   //产品名称，该参数表示oss存储路径和部署服务器路径中微服务名
def WAR_NAME = 'Web-Component-0.0.1.war'  //WAR包名称
...
//def Script_path = "prod/$Group/Prod-$Group-$PRODUCTION-Pipeline.groovy"
...
pipelineJob("$PIPELINE_JOB_NAME") {
  ...
  definition {
    cpsScm {
      ...
      scriptPath("jenkins-pipelines/prodd/plugins/Prod-plugins-ms-web-component-Pipeline.groovy")
    }
  }
}