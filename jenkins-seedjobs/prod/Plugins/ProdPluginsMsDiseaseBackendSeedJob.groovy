...
def COMPILE_JOB_NAME = Folder+'/'+'Prod-Plugins-MsDiseaseBackend-Compile' //编译job名称，不可以使用中文
def DEPLOY_JOB_NAME1 = Folder+'/'+'Prod-Plugins-MsDiseaseBackend-Deploy1' //发布job名称，不可以使用中文
def DEPLOY_JOB_NAME2 = Folder+'/'+'Prod-Plugins-MsDiseaseBackend-Deploy2' //发布job名称，不可以使用中文
def PIPELINE_JOB_NAME = Folder+'/'+'Prod-Plugins-MsDiseaseBackend-Pipeline'

def GITURL = 'http:// <git>/scm/qy-components/backend-medicine.git' //git仓库地址
...
def PRODUCTION = 'ms-medicine-backend'   //产品名称，该参数表示oss存储路径和部署服务器路径中微服务名
def WAR_NAME = 'Backend-Medicine-0.0.1.war'  //WAR包名称
...
pipelineJob("$PIPELINE_JOB_NAME") {
  ...
  definition {
    cpsScm {
      ...
      scriptPath("jenkins-pipelines/prodd/plugins/Prod-plugins-ms-diseasebackend-Pipeline.groovy")
    }
  }
}