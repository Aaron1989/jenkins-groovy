...
def COMPILE_JOB_NAME = Folder+'/'+'Prod-Plugins-MsDoctor-Compile' //编译job名称，不可以使用中文
def DEPLOY_JOB_NAME1 = Folder+'/'+'Prod-Plugins-MsDoctor-Deploy1' //发布job名称，不可以使用中文
def DEPLOY_JOB_NAME2 = Folder+'/'+'Prod-Plugins-MsDoctor-Deploy2' //发布job名称，不可以使用中文
def DEPLOY_JOB_NAME3 = Folder+'/'+'Prod-Plugins-MsDoctor-Deploy3' //发布job名称，不可以使用中文
def PIPELINE_JOB_NAME = Folder+'/'+'Prod-Plugins-MsDoctor-Pipeline'

def GITURL = 'http://h.quyiyuan.com/scm/qy-components/ms-doctor.git' //git仓库地址
...
def PRODUCTION = 'ms-doctor'   //产品名称，该参数表示oss存储路径和部署服务器路径中微服务名
def WAR_NAME = 'MS-Doctor-0.0.1.war'  //WAR包名称


def APP_NAME = 'ms-doctor'  //微服务名称,一般固定在代码里，若不修改则为空
...
pipelineJob("$PIPELINE_JOB_NAME") {
  ...
  definition {
    cpsScm {
      ...
      scriptPath("jenkins-pipelines/prodd/plugins/Prod-plugins-ms-doctor-Pipeline.groovy")
    }
  }
}