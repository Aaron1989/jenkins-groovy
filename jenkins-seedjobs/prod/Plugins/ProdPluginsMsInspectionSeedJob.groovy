...
def COMPILE_JOB_NAME = Folder+'/'+'Prod-Plugins-MsInspection-Compile' //编译job名称，不可以使用中文
def DEPLOY_JOB_NAME1 = Folder+'/'+'Prod-Plugins-MsInspection-Deploy1' //发布job名称，不可以使用中文
def DEPLOY_JOB_NAME2 = Folder+'/'+'Prod-Plugins-MsInspection-Deploy2' //发布job名称，不可以使用中文
def PIPELINE_JOB_NAME = Folder+'/'+'Prod-Plugins-MsInspection-Pipeline'

def GITURL = 'http://h.quyiyuan.com/scm/qy-components/ms-inspection.git' //git仓库地址
...
def PRODUCTION = 'ms-inspection'   //产品名称，该参数表示oss存储路径和部署服务器路径中微服务名
def WAR_NAME = 'MS-Inspection-0.0.1.war'  //WAR包名称
...
pipelineJob("$PIPELINE_JOB_NAME") {
  ...
  definition {
    cpsScm {
      ...
      scriptPath("jenkins-pipelines/prodd/plugins/Prod-plugins-ms-inspection-Pipeline.groovy")
    }
  }
}