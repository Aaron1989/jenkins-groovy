...
def COMPILE_JOB_NAME = Folder+'/'+'Prod-Plugins-MsInspectionBackend-Compile' //编译job名称，不可以使用中文
def DEPLOY_JOB_NAME1 = Folder+'/'+'Prod-Plugins-MsInspectionBackend-Deploy1' //发布job名称，不可以使用中文
def DEPLOY_JOB_NAME2 = Folder+'/'+'Prod-Plugins-MsInspectionBackend-Deploy2' //发布job名称，不可以使用中文
def PIPELINE_JOB_NAME = Folder+'/'+'Prod-Plugins-MsInspectionBackend-Pipeline'

def GITURL = 'http://h.quyiyuan.com/scm/qy-components/backend-inspection.git' //git仓库地址
...
def PRODUCTION = 'ms-inspection-backend'   //产品名称，该参数表示oss存储路径和部署服务器路径中微服务名
...
pipelineJob("$PIPELINE_JOB_NAME") {
  ...
  definition {
    cpsScm {
      ...
      scriptPath("jenkins-pipelines/prodd/plugins/Prod-plugins-ms-inspectionbackend-Pipeline.groovy")
    }
  }
}