...
def COMPILE_JOB_NAME = Folder+'/'+'Prod-Plugins-SyncCache-Compile' //编译job名称，不可以使用中文
...
def PRODUCTION = 'sync-cache'   //产品名称，该参数表示oss存储路径和部署服务器路径中微服务名
def WAR_NAME = 'Sync-Cache-0.0.1.war'  //WAR包名称
...
pipelineJob("$PIPELINE_JOB_NAME") {
  ...
  definition {
    cpsScm {
      ...
      scriptPath("jenkins-pipelines/prodd/plugins/Prod-plugins-sync-cache-Pipeline.groovy")
    }
  }
}