...
def deployEcsIPs = [' ',' ',' ',' ',' ','  ']
def DEPLOY_JOB_NAME =['Prod-Web-Max-Compute-Build', 'Prod-Web-Max-Compute-Deploy']
def Pipeline_Job_name = 'Prod-Max-Compute-Pipeline'
def gitUrl = "http://h.quyiyuan.com/scm/bigdata/max-compute.git"
...
def OSSObject = "prod/bigdata/max-compute"
def artifacts = "prediction-app.tar"
def objectPre = "prod/bigdata/max-compute/\$TAG"
...
def Folder = "Prod-Web"
def compileJobName = Folder+ '/' +'Prod-Web-Max-Compute-Compile'
...
def DockerAppName = "max-compute"
def DockerComposeTemplate = "marketing/prediction-app/docker-compose-deploy.yml"
...
//    mavenInstallation('maven-3.2.5')
    ...
    steps {
        shell(
                "tar -czf prediction-app.tar  PythonApp"
//                "python /opt/OSS_Python_API/osscmd uploadfromdir PythonApp/ oss://deployment/prod/bigdata/max-compute/\${TAG}/prediction-app/ --check_md5=true --check_point=cp.txt"
        )
    }
...
freeStyleJob(Folder+ '/' +DEPLOY_JOB_NAME[0]) {
    ...
    steps{
        shell(
                ...
                '''DOCKER_HOST=''\ndocker-compose -f marketing/prediction-app/docker-compose-build.yml build --no-cache\ndocker push registry-internal.cn-qingdao.aliyuncs.com/quyiyuan/prediction-app:$DOCKER_TAG_VERSION'''
...
//        stringParam ('OLD_VERSION',"")
    ...
    definition {
        cpsScm {
            ...
            scriptPath("jenkins-pipelines/prodd/Web/Prod-Max-Compute-Pipeline")
        }
    }
}
