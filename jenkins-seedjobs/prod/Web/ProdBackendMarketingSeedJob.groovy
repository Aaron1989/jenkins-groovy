...
def deployEcsIPs = [' ',' ',' ',' ',' ','  ']
def DEPLOY_JOB_NAME =['Prod-Web-Backend-Marketing-Build', 'Prod-Web-Backend-Marketing-Deploy']
def Pipeline_Job_name = 'Prod-Backend-Marketing-Pipeline'
def gitUrl = "http://h.quyiyuan.com/scm/bigdata/backend-marketing.git"
...
def OSSObject = "prod/marketing/backend-marketing"
def artifacts = "target/*.jar"
def objectPre = "prod/marketing/backend-marketing/\$TAG"
...
def Folder = "Prod-Web"
def compileJobName = Folder+ '/' +'Prod-Web-Backend-Marketing-Compile'
...
def DockerAppName = "backend-marketing"
def DockerComposeTemplate = "marketing/backend-marketing/docker-compose-deploy.yml"
...
freeStyleJob(Folder+ '/' +DEPLOY_JOB_NAME[0]) {
    ...
    steps{
        shell(
                ...
                '''DOCKER_HOST=''\ndocker-compose -f marketing/backend-marketing/docker-compose-build.yml build --no-cache\ndocker push registry-internal.cn-qingdao.aliyuncs.com/quyiyuan/backend-marketing:$DOCKER_TAG_VERSION'''
...
//        stringParam ('OLD_VERSION',"")
    ...
    definition {
        cpsScm {
            ...
            scriptPath("jenkins-pipelines/prodd/Web/Prod-Backend-Marketing-Pipeline")
        }
    }
}
