...
def deployEcsIPs = [' ',' ',' ',' ',' ','  ']
def DEPLOY_JOB_NAME =['Prod-Web-Backend-Datax-Build', 'Prod-Web-Backend-Datax-Deploy']
def Pipeline_Job_name = 'Prod-Backend-Datax-Pipeline'
def gitUrl = "http://h.quyiyuan.com/scm/bigdata/dataxspringserver.git"
...
def OSSObject = "prod/marketing/backend-datax"
def artifacts = "target/*.jar"
def objectPre = "prod/marketing/backend-datax/\$TAG"
...
def Folder = "Prod-Web"
def compileJobName = Folder+ '/' +'Prod-Web-Backend-Datax-Compile'
...
def DockerAppName = "backend-datax"
def DockerComposeTemplate = "marketing/backend-datax/docker-compose-deploy.yml"
...
freeStyleJob(Folder+ '/' +DEPLOY_JOB_NAME[0]) {
    ...
    steps{
        shell(
                ...
                '''DOCKER_HOST=''\ndocker-compose -f marketing/backend-datax/docker-compose-build.yml build --no-cache\ndocker push registry-internal.cn-qingdao.aliyuncs.com/quyiyuan/backend-datax:$DOCKER_TAG_VERSION'''
...
//        stringParam ('OLD_VERSION',"")
    ...
    definition {
        cpsScm {
            ...
            scriptPath("jenkins-pipelines/prodd/Web/Prod-Backend-Datax-Pipeline")
        }
    }
}
