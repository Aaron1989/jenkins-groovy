...
def deployEcsIPs = [' ',' ',' ',' ',' ','  ']
def DEPLOY_JOB_NAME =['Prod-Web-Qy-Advertisement-Build', 'Prod-Web-Qy-Advertisement-Deploy']
def Pipeline_Job_name = 'Prod-Qy-Advertisement-Pipeline'
def gitUrl = "http://h.quyiyuan.com/scm/bigdata/qy-advertisement.git"
...
def OSSObject = "prod/bigdata/qy-advertisement"
def artifacts = "target/*.jar"
def objectPre = "prod/bigdata/qy-advertisement/\$TAG"
...
def Folder = "Prod-Web"
def compileJobName = Folder+ '/' +'Prod-Web-Qy-Advertisement-Compile'
...
def DockerAppName = "qy-advertisement"
def DockerComposeTemplate = "marketing/qy-advertisement/docker-compose-deploy.yml"
...
freeStyleJob(Folder+ '/' +DEPLOY_JOB_NAME[0]) {
    ...
    steps{
        shell(
                ...
                '''DOCKER_HOST=''\ndocker-compose -f marketing/qy-advertisement/docker-compose-build.yml build --no-cache\ndocker push registry-internal.cn-qingdao.aliyuncs.com/quyiyuan/qy-advertisement:$DOCKER_TAG_VERSION'''
...
//        stringParam ('OLD_VERSION',"")
    ...
    definition {
        cpsScm {
            ...
            scriptPath("jenkins-pipelines/prodd/Web/Prod-Qy-Advertisement-Pipeline")
        }
    }
}
