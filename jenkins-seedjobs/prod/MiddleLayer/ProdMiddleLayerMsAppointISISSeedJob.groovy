...
def COMPILE_JOB_NAME = Folder+'/'+'Prod-MiddleLayer-MsAppointIsis-Compile' //编译job名称，不可以使用中文
def DEPLOY_JOB_NAME1 = Folder+'/'+'Gray-MiddleLayer-MsAppointIsis-Deploy' //发布job名称，不可以使用中文
def DEPLOY_JOB_NAME2 = Folder+'/'+'Prod-MiddleLayer-MsAppointIsis-Deploy1' //发布job名称，不可以使用中文
def DEPLOY_JOB_NAME3 = Folder+'/'+'Prod-MiddleLayer-MsAppointIsis-Deploy2' //发布job名称，不可以使用中文
def DEPLOY_JOB_NAME4 = Folder+'/'+'Prod-MiddleLayer-MsAppointIsis-Deploy3' //发布job名称，不可以使用中文
def PIPELINE_JOB_NAME1 = Folder+'/'+'Prod-MiddleLayer-MsAppointIsis-Pipeline'
def ECS_IP = [' ', ' ', ' ', ' ']

def pipelineGitUrl = 'https://github.com/Aaron1989/jenkins-groovy.git'

def GITURL = 'http://h.quyiyuan.com/scm/qy-components/backend-appoint.git' //git仓库地址
...
def PRODUCTION = 'backend-appoint'   //产品名称，该参数表示oss存储路径和部署服务器路径中微服务名
def WAR_NAME = 'backend-appoint-app.war'  //jar包名称
def WAR_PATH = 'webapp/target'
...
def ScriptPath = 'prod/'+GROUP+'/Prod-MiddleLayer-MsAppointIsis-Pipeline.groovy'
...
freeStyleJob("$DEPLOY_JOB_NAME1") {
    ...
    parameters {
        ...
        stringParam ('SSL_PORT','8043')      //redirectPort指定服务器正在处理http请求时收到一个SSL传输请求后重定向的端口号
        stringParam ('AJP_PORT','8009')   //第二个 指定服务器端要创建的端口号，并在这个断口监听来自客户端的请求
        textParam ('JPS_PARAMETER','{\'PRODUCTION\':\'backend-appoint\',\'GROUP\':\'middlelayer\',\'ENV\':\'release\',"TAG":"$TAG","WAR_NAME":"$WAR_NAME","EXPORT":"","TOMCAT_PORT":"$TOMCAT_PORT","SERVER_PORT":"$SERVER_PORT","SSL_PORT":"$SSL_PORT","AJP_PORT":"$AJP_PORT","SPRING_CONFIG_PROFILE":"release"}')
