...
def COMPILE_JOB_NAME = Folder+'/'+'Prod-MiddleLayer-MsAppoint-Compile' //编译job名称，不可以使用中文
def DEPLOY_JOB_NAME1 = Folder+'/'+'Gray-MiddleLayer-MsAppoint-Deploy1' //发布job名称，不可以使用中文
def DEPLOY_JOB_NAME2 = Folder+'/'+'Prod-MiddleLayer-MsAppoint-Deploy1' //发布job名称，不可以使用中文
def DEPLOY_JOB_NAME3 = Folder+'/'+'Prod-MiddleLayer-MsAppoint-Deploy2' //发布job名称，不可以使用中文
def DEPLOY_JOB_NAME4 = Folder+'/'+'Prod-MiddleLayer-MsAppoint-Deploy3' //发布job名称，不可以使用中文
...
def PRODUCTION = 'ms-appoint'   //产品名称，该参数表示oss存储路径和部署服务器路径中微服务名
def WAR_NAME = 'ms-appoint-1.0-SNAPSHOT.jar'  //jar包名称
def JPS_PARAMETER_GRAY = '''{'PRODUCTION':'ms-appoint','GROUP':'middlelayer','APP_OPTS':'--client.isis.url=http:// :8080/backend-appoint-app/restful/services --spring.application.name=ms-appoint-pre','ENV':'release','JAVA_OPTS':'-Xms1g -Xmx1g','EUREKA_OPTS':'--eureka.instance.metadataMap.OldVersion=$OLD_VERSION --eureka.instance.metadataMap.LastVersion=$TAG --eureka.instance.metadataMap.WarName=$WAR_NAME --eureka.instance.metadataMap.Date=`date "+%Y-%m-%d"` --eureka.instance.metadataMap.Time=`date "+%H:%M:%S "` --eureka.instance.metadataMap.JavaOptXms=1g --eureka.instance.metadataMap.JavaOptXmx=1g',"TAG":"$TAG","WAR_NAME":"$WAR_NAME","EXPORT":""}'''
def JPS_PARAMETER_PROD = '''{'PRODUCTION':'ms-appoint','GROUP':'middlelayer','APP_OPTS':'--spring.application.name=ms-appoint','ENV':'release','JAVA_OPTS':'-Xms2048M -Xmx2048M','EUREKA_OPTS':'--eureka.instance.metadataMap.OldVersion=$OLD_VERSION --eureka.instance.metadataMap.LastVersion=$TAG --eureka.instance.metadataMap.WarName=$WAR_NAME --eureka.instance.metadataMap.Date=`date "+%Y-%m-%d"` --eureka.instance.metadataMap.Time=`date "+%H:%M:%S "` --eureka.instance.metadataMap.JavaOptXms=2048M --eureka.instance.metadataMap.JavaOptXmx=2048M',"TAG":"$TAG","WAR_NAME":"$WAR_NAME","EXPORT":""}'''


def APP_NAME = 'ms-appoint'  //微服务名称,一般固定在代码里，若不修改则为空
...
def ScriptPath = 'prod/'+GROUP+'/Prod-MiddleLayer-MsAppoint-Pipeline.groovy'
...
freeStyleJob("$DEPLOY_JOB_NAME2") {
    ...
    parameters {
        ...
        stringParam ('OLD_VERSION',"")
        stringParam ('WAR_NAME',"$WAR_NAME")
        textParam ('JPS_PARAMETER', JPS_PARAMETER_PROD)
//        textParam ('JPS_PARAMETER',"{'PRODUCTION':'$PRODUCTION','GROUP':'$GROUP','APP_OPTS':'$APP_OPTS','ENV':'release','JAVA_OPTS':'$JAVA_OPTS','EUREKA_OPTS':'$EUREKA_OPTS',"+'"TAG":"$TAG","WAR_NAME":"$WAR_NAME","EXPORT":""}')
        stringParam ('SHELL_NAME',"$SHELL_NAME")
