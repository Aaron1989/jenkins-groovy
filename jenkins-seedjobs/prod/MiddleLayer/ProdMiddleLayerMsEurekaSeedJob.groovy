...
def COMPILE_JOB_NAME = Folder+'/'+'Prod-MiddleLayer-MsEureka-Compile' //编译job名称，不可以使用中文
def DEPLOY_JOB_NAME1 = Folder+'/'+'Prod-MiddleLayer-MsEureka-Deploy1' //发布job名称，不可以使用中文
def DEPLOY_JOB_NAME2 = Folder+'/'+'Prod-MiddleLayer-MsEureka-Deploy2' //发布job名称，不可以使用中文
def PIPELINE_JOB_NAME1 = Folder+'/'+'Prod-MiddleLayer-MsEureka-Pipeline'
def ECS_IP = [' ', ' ']

def GITURL = 'http://h.quyiyuan.com/scm/qy-components/eureka-server.git' //git仓库地址
...
def PRODUCTION = 'ms-eureka'   //产品名称，该参数表示oss存储路径和部署服务器路径中微服务名
def WAR_NAME = 'eureka-server-1.0-SNAPSHOT.jar'  //jar包名称


//
def APP_NAME = 'ms-eureka'  //微服务名称,一般固定在代码里，若不修改则为空
...
def ScriptPath = 'prod/'+GROUP+'/Prod-MiddleLayer-MsEureka-Pipeline.groovy'
