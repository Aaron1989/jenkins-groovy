...
def COMPILE_JOB_NAME = Folder+'/'+'Prod-MiddleLayer-MsGuide-Compile' //编译job名称，不可以使用中文
def DEPLOY_JOB_NAME1 = Folder+'/'+'Prod-MiddleLayer-MsGuide-Deploy' //发布job名称，不可以使用中文
def PIPELINE_JOB_NAME1 = Folder+'/'+'Prod-MiddleLayer-MsGuide-Pipeline'
def ECS_IP = [' ']

def GITURL = 'http://h.quyiyuan.com/scm/ms/ms-guide.git' //git仓库地址
...
def PRODUCTION = 'ms-guide'   //产品名称，该参数表示oss存储路径和部署服务器路径中微服务名
def WAR_NAME = 'MS-Guide-1.0-SNAPSHOT.war'  //jar包名称

def APP_NAME = 'ms-guide'  //微服务名称,一般固定在代码里，若不修改则为空
...
def ScriptPath = 'prod/'+GROUP+'/Prod-MiddleLayer-MsGuide-Pipeline'
