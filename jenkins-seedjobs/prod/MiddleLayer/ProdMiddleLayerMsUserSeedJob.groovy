...
def COMPILE_JOB_NAME = Folder+'/'+'Prod-MiddleLayer-MsUser-Compile' //编译job名称，不可以使用中文
def DEPLOY_JOB_NAME1 = Folder+'/'+'Prod-MiddleLayer-MsUser-Deploy1' //发布job名称，不可以使用中文
def DEPLOY_JOB_NAME2 = Folder+'/'+'Prod-MiddleLayer-MsUser-Deploy2' //发布job名称，不可以使用中文
def PIPELINE_JOB_NAME = Folder+'/'+'Prod-MiddleLayer-MsUser-Pipeline'
def ECS_IP = [' ', ' ']

def pipelineGitUrl = 'https://github.com/Aaron1989/jenkins-groovy.git'

def GITURL = 'http://h.quyiyuan.com/scm/qy-components/ms-user.git' //git仓库地址
...
def PRODUCTION = 'ms-user'   //产品名称，该参数表示oss存储路径和部署服务器路径中微服务名
def WAR_NAME = 'ms-user-1.0-SNAPSHOT.war'  //jar包名称


//
def APP_NAME = 'ms-user'  //微服务名称,一般固定在代码里，若不修改则为空
...
def ScriptPath = 'prod/'+GROUP+'/Prod-MiddleLayer-MsUser-Pipeline.groovy'
