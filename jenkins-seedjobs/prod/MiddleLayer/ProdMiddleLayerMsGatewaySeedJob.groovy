...
def COMPILE_JOB_NAME = Folder+'/'+'Prod-MiddleLayer-MsGateway-Compile' //编译job名称，不可以使用中文
def DEPLOY_JOB_NAME1 = Folder+'/'+'Prod-MiddleLayer-MsGateway-Deploy1' //发布job名称，不可以使用中文
def DEPLOY_JOB_NAME2 = Folder+'/'+'Prod-MiddleLayer-MsGateway-Deploy2' //发布job名称，不可以使用中文
def DEPLOY_JOB_NAME3 = Folder+'/'+'Prod-MiddleLayer-MsGateway-Deploy3' //发布job名称，不可以使用中文
def DEPLOY_JOB_NAME4 = Folder+'/'+'Prod-MiddleLayer-MsGateway-Deploy4' //发布job名称，不可以使用中文
def DEPLOY_JOB_NAME5 = Folder+'/'+'Prod-MiddleLayer-MsGateway-Deploy5' //发布job名称，不可以使用中文
def DEPLOY_JOB_NAME6 = Folder+'/'+'Prod-MiddleLayer-MsGateway-Deploy6' //发布job名称，不可以使用中文

def PIPELINE_JOB_NAME1 = Folder+'/'+'Prod-MiddleLayer-MsGateway-Pipeline'
def ECS_IP = [' ', ' ',' ',' ',' ',' ']

def GITURL = 'http://h.quyiyuan.com/scm/qy-components/ms-gateway.git' //git仓库地址
...
def WAR_NAME = 'qy-gateway-1.0-SNAPSHOT.war'  //jar包名称


//
def APP_NAME = 'ms-gateway'  //微服务名称,一般固定在代码里，若不修改则为空
...
def ScriptPath = 'prod/'+GROUP+'/Prod-MiddleLayer-MsGateway-Pipeline'
...
mavenJob("${COMPILE_JOB_NAME}") {
    ...
    publishers {
        aliyunOSSPublisher {
            ...
            filesPath('qy-gateway/target/$WAR_NAME')
...
freeStyleJob("$DEPLOY_JOB_NAME5") {
    ...
    steps {
        jumpserverHttpsBuilder {
            ...
            jpsURL('生产环境jps')
            appName("$PRODUCTION")   //记录jumpserver上发布的微服务名称
            ecsSearchOption(ECS_IP[4])
        }
    }
}

freeStyleJob("$DEPLOY_JOB_NAME6") {
    logRotator(3, 3)
    parameters {
        password {
            name('JPS_PWD')
