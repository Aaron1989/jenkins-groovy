...
def COMPILE_JOB_NAME = Folder+'/'+'Prod-MiddleLayer-MsConfig-Compile' //编译job名称，不可以使用中文
def DEPLOY_JOB_NAME = Folder+'/'+'Prod-MiddleLayer-MsConfig-Deploy' //发布job名称，不可以使用中文
...
def PRODUCTION = 'ms-config'   //产品名称，该参数表示oss存储路径和部署服务器路径中微服务名
def WAR_NAME = 'ms-config-0.0.1-SNAPSHOT.jar'  //jar包名称

def APP_NAME = 'ms-config'  //微服务名称,一般固定在代码里，若不修改则为空
...
def EXPORT = "export CONFIG_ISIS_USERNAME=isis-module-security-admin export CONFIG_ISIS_PASSWORD=IUImzsX92hpn"
...
def ScriptPath = 'prod/'+GROUP+'/Prod-MiddleLayer-MsConfig-Pipeline.groovy'
...
freeStyleJob("$DEPLOY_JOB_NAME") {
    ...
    parameters {
        ...
        stringParam ('WAR_NAME',"$WAR_NAME")
        textParam ('JPS_PARAMETER',"{'PRODUCTION':'$PRODUCTION','GROUP':'$GROUP','APP_OPTS':'$APP_OPTS','ENV':'release','JAVA_OPTS':'$JAVA_OPTS','EUREKA_OPTS':'$EUREKA_OPTS','EXPORT':'$EXPORT',"+'"TAG":"$TAG","WAR_NAME":"$WAR_NAME"}')
