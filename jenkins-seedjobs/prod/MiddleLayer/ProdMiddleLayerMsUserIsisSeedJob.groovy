...
def COMPILE_JOB_NAME = Folder+'/'+'Prod-MiddleLayer-MsUserIsis-Compile' //编译job名称，不可以使用中文
def DEPLOY_JOB_NAME1 = Folder+'/'+'Prod-MiddleLayer-MsUserIsis-Deploy1' //发布job名称，不可以使用中文
def DEPLOY_JOB_NAME2 = Folder+'/'+'Prod-MiddleLayer-MsUserIsis-Deploy2' //发布job名称，不可以使用中文
def PIPELINE_JOB_NAME1 = Folder+'/'+'Prod-MiddleLayer-MsUserIsis-Pipeline'
def ECS_IP = [' ', ' ']

def GITURL = 'http://h.quyiyuan.com/scm/qy-components/backend-user.git' //git仓库地址
...
def PRODUCTION = 'ms-user-isis'   //产品名称，该参数表示oss存储路径和部署服务器路径中微服务名
def WAR_NAME = 'backend-user.war'  //jar包名称
...
def ScriptPath = 'prod/'+GROUP+'/Prod-MiddleLayer-MsUserIsis-Pipeline.groovy'
...
freeStyleJob("$DEPLOY_JOB_NAME1") {
    ...
    parameters {
        ...
        stringParam ('WAR_NAME',"$WAR_NAME")
        stringParam ('SERVER_PORT','8081')  //指定一个端口，这个端口负责监听关闭tomcat 的请求
        ...
        textParam ('JPS_PARAMETER',"{'PRODUCTION':'$PRODUCTION','GROUP':'$GROUP','ENV':'release',"+'"TAG":"$TAG","WAR_NAME":"$WAR_NAME","EXPORT":"","TOMCAT_PORT":"$TOMCAT_PORT","SERVER_PORT":"$SERVER_PORT","SSL_PORT":"$SSL_PORT","AJP_PORT":"$AJP_PORT"}')
