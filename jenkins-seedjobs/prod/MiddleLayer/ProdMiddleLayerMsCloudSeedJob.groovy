...
def COMPILE_JOB_NAME = Folder+'/'+'Prod-MiddleLayer-MsCloud-Compile' //编译job名称，不可以使用中文
def DEPLOY_JOB_NAME1 = Folder+'/'+'Prod-MiddleLayer-MsCloud-Deploy1' //发布job名称，不可以使用中文
def DEPLOY_JOB_NAME2 = Folder+'/'+'Prod-MiddleLayer-MsCloud-Deploy2' //发布job名称，不可以使用中文
def PIPELINE_JOB_NAME1 = Folder+'/'+'Prod-MiddleLayer-MsCloud-Pipeline'
def ECS_IP = [' ', ' ']
...
def WAR_NAME = 'Web-1.0-SNAPSHOT.war'  //jar包名称


//
def APP_NAME = 'ms-cloud'  //微服务名称,一般固定在代码里，若不修改则为空
...
def ScriptPath = 'prod/'+GROUP+'/Prod-MiddleLayer-MsCloud-Pipeline.groovy'
