...
def COMPILE_JOB_NAME = Folder+'/'+'Prod-MiddleLayer-MsGuideIsis-Compile' //编译job名称，不可以使用中文
def DEPLOY_JOB_NAME1 = Folder+'/'+'Prod-MiddleLayer-MsGuideIsis-Deploy' //发布job名称，不可以使用中文
def PIPELINE_JOB_NAME1 = Folder+'/'+'Prod-MiddleLayer-MsGuideIsis-Pipeline'
def ECS_IP = [' ']

def GITURL = 'http://h.quyiyuan.com/scm/archive/isis-guide.git'//git仓库地址
...
def PRODUCTION = 'ms-guide-isis'   //产品名称，该参数表示oss存储路径和部署服务器路径中微服务名
def WAR_NAME = 'QY-Intelligent-Guide.war'  //jar包名称
def WAR_PATH = 'webapp/target'

def ScriptPath = 'prod/'+GROUP+'/Prod-MiddleLayer-MsGuideIsis-Pipeline'
