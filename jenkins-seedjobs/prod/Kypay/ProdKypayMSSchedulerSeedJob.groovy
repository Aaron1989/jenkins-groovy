...
def COMPILE_JOB_NAME = Folder+'/'+'Prod-Kypay-MS-Scheduler-Compile' //编译job名称，不可以使用中文
def DEPLOY_JOB_NAME = Folder+'/'+'Prod-Kypay-MS-Scheduler-Deploy' //发布job名称，不可以使用中文
def PIPELINE_JOB_NAME1 = Folder+'/'+'Prod-Kypay-MS-Scheduler-Pipeline'

def GITURL = 'http://h.quyiyuan.com/scm/qypay/ms-scheduler.git' //git仓库地址
...
def GROUP = 'kypay'  //所属组
def PRODUCTION = 'ms-scheduler'   //产品名称，该参数表示oss存储路径和部署服务器路径中微服务名
def WAR_NAME = 'ms-scheduler.war'  //WAR包名称
...
def Xms = '256M'
def Xmx = '512M'
def JAVA_OPTS = "-Xms${Xms} -Xmx${Xmx} --qy.log.path=/usr/local/log"//java启动参数，若有需要请自行添加
...
pipelineJob("$PIPELINE_JOB_NAME1") {
  ...
  definition {
    cpsScm {
      ...
      scriptPath("jenkins-pipelines/prodd/Kypay/Prod-Kypay-MS-Scheduler-Pipeline.groovy")
    }
  }
}
