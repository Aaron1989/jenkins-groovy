...
def COMPILE_JOB_NAME = Folder+'/'+'Prod-Kypay-MS-QuyiAcct-Compile' //编译job名称，不可以使用中文
def DEPLOY_JOB_NAME1 = Folder+'/'+'Prod-Kypay-MS-QuyiAcct-Deploy1' //发布job名称，不可以使用中文
def DEPLOY_JOB_NAME2 = Folder+'/'+'Prod-Kypay-MS-QuyiAcct-Deploy2' //发布job名称，不可以使用中文
def PIPELINE_JOB_NAME1 = Folder+'/'+'Prod-Kypay-MS-QuyiAcct-Pipeline'

def GITURL = 'http://h.quyiyuan.com/scm/qypay/ms-quyiacct.git' //git仓库地址
...
def GROUP = 'kypay'  //所属组
def PRODUCTION = 'ms-quyiacct'   //产品名称，该参数表示oss存储路径和部署服务器路径中微服务名
def WAR_NAME = 'ms-quyiacct.war'  //WAR包名称
...
pipelineJob("$PIPELINE_JOB_NAME1") {
  ...
  definition {
    cpsScm {
      ...
      scriptPath("jenkins-pipelines/prodd/Kypay/Prod-Kypay-MS-QuyiAcct-Pipeline.groovy")
    }
  }
}