...
def COMPILE_JOB_NAME = Folder+'/'+'Prod-Hms-HMS-Component-Compile' //编译job名称，不可以使用中文
def DEPLOY_JOB_NAME = Folder+'/'+'Prod-Hms-HMS-Component-Deploy' //发布job名称，不可以使用中文
...
def PRODUCTION = 'ms-hms-component'   //产品名称，该参数表示oss存储路径和部署服务器路径中微服务名
def WAR_NAME = 'ms-hms-component-1.0.0.war'  //WAR包名称

def EXPORT =''
def APP_NAME = 'ms-hms-component'  //微服务名称,一般固定在代码里，若不修改则为空
...
pipelineJob("$PIPELINE_JOB_NAME") {
    ...
    definition {
        cpsScm {
            ...
            scriptPath("jenkins-pipelines/prodd/NewHms/Prod-Hms-HMS-Component-Pipeline.groovy")
        }
    }
}
