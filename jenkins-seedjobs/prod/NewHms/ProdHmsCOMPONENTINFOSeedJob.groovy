//发布job初始化
def Folder = 'Prod-NewHms'

def COMPILE_JOB_NAME = Folder+'/'+'Prod-Hms-MS-HMS-COMPONENT-INFO-Compile' //编译job名称，不可以使用中文
def DEPLOY_JOB_NAME = Folder+'/'+'Prod-Hms-MS-HMS-COMPONENT-INFO-Deploy' //发布job名称，不可以使用中文
def PIPELINE_JOB_NAME =  Folder+'/'+'Prod-Hms-MS-HMS-COMPONENT-INFO-Pipeline'
def ECS_IP = [' ']

def GITURL = 'http://h.quyiyuan.com/scm/ysapp/ms-hms-component-info.git' //git仓库地址
...
def GROUP = 'newhms'  //所属组
def PRODUCTION = 'ms-hms-component-info'   //产品名称，该参数表示oss存储路径和部署服务器路径中微服务名
def WAR_NAME = 'ms-hms-component-info-0.0.1-SNAPSHOT.jar'  //WAR包名称

def EXPORT =''
def APP_NAME = 'ms-hms-component-info'  //微服务名称,一般固定在代码里，若不修改则为空
...
pipelineJob("$PIPELINE_JOB_NAME") {
    ...
    definition {
        cpsScm {
            ...
            scriptPath("jenkins-pipelines/prodd/NewHms/Prod-Hms-MS-HMS-COMPONENT-INFO-Pipeline.groovy")
        }
    }
}
