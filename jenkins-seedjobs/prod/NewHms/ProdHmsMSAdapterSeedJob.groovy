...
def COMPILE_JOB_NAME = Folder+'/'+'Prod-Hms-MS-Adapter-Compile' //编译job名称，不可以使用中文
def DEPLOY_JOB_NAME = Folder+'/'+'Prod-Hms-MS-Adapter-Deploy' //发布job名称，不可以使用中文
def PIPELINE_JOB_NAME =  Folder+'/'+'Prod-Hms-MS-Adapter-Pipeline'
def ECS_IP = [' ']

def GITURL = 'http://h.quyiyuan.com/scm/ysapp/ms-adapter.git' //git仓库地址
...
def PRODUCTION = 'ms-adapter'    //产品名称，该参数表示oss存储路径和部署服务器路径中微服务名
def WAR_NAME = 'ms-adapter-1.0.0.jar'  //WAR包名称

def EXPORT =''
def APP_NAME = 'ms-adapter-transfer'  //微服务名称,一般固定在代码里，若不修改则为空
def APP_OPTS = "--spring.application.name=$APP_NAME"   //此参数用于强制注入微服务名称，一般会固定在代码中，若不修改该参数为空

def Xms = '512M'
def Xmx = '1024M'
def JAVA_OPTS = "-Xms${Xms} -Xmx${Xmx} --SPRING_CONFIG_URI=http:// :7777"//java启动参数，若有需要请自行添加
...
pipelineJob("$PIPELINE_JOB_NAME") {
    ...
    definition {
        cpsScm {
            ...
            scriptPath("jenkins-pipelines/prodd/NewHms/Prod-Hms-MS-Adapter-Pipeline.groovy")
        }
    }
}
