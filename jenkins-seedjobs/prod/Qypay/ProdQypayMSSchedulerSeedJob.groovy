...
def COMPILE_JOB_NAME = Folder+'/'+'Prod-Qypay-MS-Scheduler-Compile' //编译job名称，不可以使用中文
def DEPLOY_JOB_NAME = Folder+'/'+'Prod-Qypay-MS-Scheduler-Deploy' //发布job名称，不可以使用中文
def PIPELINE_JOB_NAME1 = Folder+'/'+'Prod-Qypay-MS-Scheduler-Pipeline'
def ECS_IP = [' ']
...
pipelineJob("$PIPELINE_JOB_NAME1") {
  ...
  definition {
    cpsScm {
      ...
      scriptPath("jenkins-pipelines/prodd/Qypay/Prod-Qypay-MS-Scheduler-Pipeline.groovy")
    }
  }
}