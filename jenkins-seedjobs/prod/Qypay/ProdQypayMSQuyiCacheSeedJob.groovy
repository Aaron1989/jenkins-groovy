...
def COMPILE_JOB_NAME = Folder+'/'+'Prod-Qypay-MS-QuyiCache-Compile' //编译job名称，不可以使用中文
def DEPLOY_JOB_NAME1 = Folder+'/'+'Prod-Qypay-MS-QuyiCache-Deploy1' //发布job名称，不可以使用中文
def DEPLOY_JOB_NAME2 = Folder+'/'+'Prod-Qypay-MS-QuyiCache-Deploy2' //发布job名称，不可以使用中文
def PIPELINE_JOB_NAME1 = Folder+'/'+'Prod-Qypay-MS-QuyiCache-Pipeline'


def GITURL = 'http://h.quyiyuan.com/scm/qypay/ms-quyicache.git' //git仓库地址
...
def GROUP = 'qypay'  //所属组
def PRODUCTION = 'ms-quyicache'   //产品名称，该参数表示oss存储路径和部署服务器路径中微服务名
def WAR_NAME = 'ms-quyicache.war'  //WAR包名称
...
mavenJob("${COMPILE_JOB_NAME}") {
    ...
    publishers {
        ...
        aliyunOSSPublisher {
            ...
            filesPath('msquyicacheserver/target/$WAR_NAME')
...
pipelineJob("$PIPELINE_JOB_NAME1") {
  ...
  definition {
    cpsScm {
      ...
      scriptPath("jenkins-pipelines/prodd/Qypay/Prod-Qypay-MS-QuyiCache-Pipeline.groovy")
    }
  }
}