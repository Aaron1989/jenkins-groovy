...
def COMPILE_JOB_NAME = Folder+'/'+'Prod-Qypay-MS-Gateway-Compile' //编译job名称，不可以使用中文
def DEPLOY_JOB_NAME1 = Folder+'/'+'Prod-Qypay-MS-Gateway-Deploy1' //发布job名称，不可以使用中文
def DEPLOY_JOB_NAME2 = Folder+'/'+'Prod-Qypay-MS-Gateway-Deploy2' //发布job名称，不可以使用中文
def PIPELINE_JOB_NAME1 = Folder+'/'+'Prod-Qypay-MS-Gateway-Pipeline'


def GITURL = 'http://h.quyiyuan.com/scm/qypay/ms-gateway.git' //git仓库地址
...
pipelineJob("$PIPELINE_JOB_NAME1") {
  ...
  definition {
    cpsScm {
      ...
      scriptPath("jenkins-pipelines/prodd/Qypay/Prod-Qypay-MS-Gateway-Pipeline.groovy")
    }
  }
}