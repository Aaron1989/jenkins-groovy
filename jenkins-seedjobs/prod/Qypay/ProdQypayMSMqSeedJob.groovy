...
def COMPILE_JOB_NAME = Folder+'/'+'Prod-Qypay-MS-Mq-Compile' //编译job名称，不可以使用中文
def DEPLOY_JOB_NAME1 = Folder+'/'+'Prod-Qypay-MS-Mq-Deploy1' //发布job名称，不可以使用中文
def DEPLOY_JOB_NAME2 = Folder+'/'+'Prod-Qypay-MS-Mq-Deploy2' //发布job名称，不可以使用中文
def PIPELINE_JOB_NAME1 = Folder+'/'+'Prod-Qypay-MS-Mq-Pipeline'



def GITURL = 'http://h.quyiyuan.com/scm/qypay/ms-mq.git' //git仓库地址
def Git_AUTH = 'qypay' //git仓库认证账号
def BRANCH = '*/release'  //发布分支

def OSS_BUCKET = 'deployment-package'  //oss存储bucket
def BR_NAME = 'release' //OSS上传环境
def ENV = 'release'  //配置文件环境
def GROUP = 'qypay'  //所属组
def PRODUCTION = 'ms-mq'   //产品名称，该参数表示oss存储路径和部署服务器路径中微服务名
def WAR_NAME = 'ms-mq.war'  //WAR包名称
...
def Xms = '256M'
def Xmx = '512M'
def JAVA_OPTS = "-Xms${Xms} -Xmx${Xmx} --qy.log.switch=open --spring.cloud.config.uri=http:// :7777 --qy.log.path=/usr/local/log"//java启动参数，若有需要请自行添加
...
pipelineJob("$PIPELINE_JOB_NAME1") {
  ...
  definition {
    cpsScm {
      ...
      scriptPath("jenkins-pipelines/prodd/Qypay/Prod-Qypay-MS-Mq-Pipeline.groovy")
    }
  }
}