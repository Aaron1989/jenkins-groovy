...
def COMPILE_JOB_NAME = Folder+'/'+'Prod-Qypay-EurekaServer-Compile' //编译job名称，不可以使用中文
def DEPLOY_JOB_NAME1 = Folder+'/'+'Prod-Qypay-EurekaServer-Deploy1' //发布job名称，不可以使用中文
def DEPLOY_JOB_NAME2 = Folder+'/'+'Prod-Qypay-EurekaServer-Deploy2' //发布job名称，不可以使用中文
def PIPELINE_JOB_NAME1 = Folder+'/'+'Prod-Qypay-EurekaServer-Pipeline'
...
def GROUP = 'qypay'  //所属组
def PRODUCTION = 'ms-eurekaserver'   //产品名称，该参数表示oss存储路径和部署服务器路径中微服务名
def WAR_NAME = 'ms-eurekaserver.war'  //WAR包名称
...
pipelineJob("$PIPELINE_JOB_NAME1") {
  ...
  definition {
    cpsScm {
      ...
      scriptPath("jenkins-pipelines/prodd/Qypay/Prod-Qypay-EurekaServer-Pipeline.groovy")
    }
  }
}