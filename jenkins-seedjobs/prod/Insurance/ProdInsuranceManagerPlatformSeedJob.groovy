...
def COMPILE_JOB_NAME = Folder+'/'+'Prod-Insurance-Manager-Platform-Compile' //编译job名称，不可以使用中文
def DEPLOY_JOB_NAME = Folder+'/'+'Prod-Insurance-Manager-Platform-Deploy' //发布job名称，不可以使用中文
def PIPELINE_JOB_NAME = Folder+'/'+'Prod-Insurance-Manager-Platform-Pipeline'

def GITURL = 'http://h.quyiyuan.com/scm/in/insurance_manger_platform.git' //git仓库地址
def Git_AUTH = '6b5de867-3f96-4162-9384-4c81e0e7063c' //git仓库认证账号
def BRANCH = ''  //发布分支

def ENV = 'release'  //配置文件环境
def GROUP = 'qybx'  //所属组
def PRODUCTION = 'ManagerPlatform'   //产品名称，该参数表示oss存储路径和部署服务器路径中微服务名
def WAR_NAME = 'insurance_manger_platform-1.0-SNAPSHOT.jar'  //WAR包名称
def OSS_BUCKET = 'deployment-package'  //oss存储bucket

def APP_OPTS = ""   //此参数用于强制注入微服务名称，一般会固定在代码中，若不修改该参数为空
def EXPORT = "export redis_pass=dasDHkkJGU12U08R"
...
pipelineJob("$PIPELINE_JOB_NAME") {
  ...
  definition {
    cpsScm {
      ...
      scriptPath("jenkins-pipelines/prodd/Insurance/Prod-insurance-manager-platform-Pipeline.groovy")
    }
  }
}