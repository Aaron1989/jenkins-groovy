...
def COMPILE_JOB_NAME = Folder+'/'+'Prod-Insurance-HB-Compile' //编译job名称，不可以使用中文
def DEPLOY_JOB_NAME1 = Folder+'/'+'Prod-Insurance-HB-Deploy1' //发布job名称，不可以使用中文
def DEPLOY_JOB_NAME2 = Folder+'/'+'Prod-Insurance-HB-Deploy2' //发布job名称，不可以使用中文
def PIPELINE_JOB_NAME = Folder+'/'+'Prod-Insurance-HB-Pipeline'

def GITURL = 'http://h.quyiyuan.com/scm/pt/insurance_hb_server.git' //git仓库地址
def Git_AUTH = 'GIT_2060' //git仓库认证账号
def BRANCH = ''  //发布分支

def ENV = 'prod'  //配置文件环境
def GROUP = 'qybx'  //所属组
def PRODUCTION = 'insurance-hb'   //产品名称，该参数表示oss存储路径和部署服务器路径中微服务名
def WAR_NAME = 'insurance-hb-1.0-SNAPSHOT.jar'  //WAR包名称
def OSS_BUCKET = 'deployment-package'  //oss存储bucket

def APP_OPTS = "--eureka.backup.zone= "   //此参数用于强制注入微服务名称，一般会固定在代码中，若不修改该参数为空
def EXPORT = "export dbusername=aus_prod export dbpassword=AjStNiMwSRK8cWT4 export redis_pass=dasDHkkJGU12U08R export DATA_ENCRYPT=false"
...
pipelineJob("$PIPELINE_JOB_NAME") {
  ...
  definition {
    cpsScm {
      ...
      scriptPath("jenkins-pipelines/prodd/Insurance/Prod-insurance-hb-Pipeline.groovy")
    }
  }
}