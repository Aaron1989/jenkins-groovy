...
def COMPILE_JOB_NAME = Folder+'/'+'Prod-Insurance-Web-Link-Compile' //编译job名称，不可以使用中文
def DEPLOY_JOB_NAME = Folder+'/'+'Prod-Insurance-Web-Link-Deploy1' //发布job名称，不可以使用中文
def DEPLOY_JOB_NAME2 = Folder+'/'+'Prod-Insurance-Web-Link-Deploy2' //发布job名称，不可以使用中文
def PIPELINE_JOB_NAME = Folder+'/'+'Prod-Insurance-Web-Link-Pipeline'

def GITURL = 'http:// <git>/scm/pt/insurance_web_link.git' //git仓库地址
def Git_AUTH = 'dingqishi' //git仓库认证账号

def ENV = 'prod'  //配置文件环境
def GROUP = 'qybx'  //所属组
def PRODUCTION = 'insurance-web-link'   //产品名称，该参数表示oss存储路径和部署服务器路径中微服务名
def WAR_NAME = 'insurance-web-link-1.0-SNAPSHOT.jar'  //WAR包名称
...
def Xmx = '1024M'
def Threads = '2'
def JAVA_OPTS = "-Xms${Xms} -Xmx${Xmx} -XX:PermSize=${Xms} -XX:MaxNewSize=${Xms} -XX:MaxPermSize=${Xmx} -XX:ParallelGCThreads=${Threads} --server.port=8771 --db.username=weblink_prod --db.password=QyBx20171226"//java启动参数，若有需要请自行添加
...
pipelineJob("$PIPELINE_JOB_NAME") {
    ...
    definition {
        cpsScm {
            ...
            scriptPath("jenkins-pipelines/prodd/Insurance/Prod-insurance-web-link-Pipeline.groovy")
        }
    }
}