...
def COMPILE_JOB_NAME = Folder+'/'+'Prod-Insurance-Ms-Oauth-Compile' //编译job名称，不可以使用中文
def DEPLOY_JOB_NAME = Folder+'/'+'Prod-Insurance-Ms-Oauth-Deploy' //发布job名称，不可以使用中文
def PIPELINE_JOB_NAME = Folder+'/'+'Prod-Insurance-Ms-Oauth-Pipeline'
...
def ENV = 'prod'  //配置文件环境
def GROUP = 'qybx'  //所属组
def PRODUCTION = 'ms-oauth'   //产品名称，该参数表示oss存储路径和部署服务器路径中微服务名
def WAR_NAME = 'oauth2-server-1.0-SNAPSHOT.jar'  //WAR包名称
def OSS_BUCKET = 'deployment-package'  //oss存储bucket

def APP_OPTS = ""   //此参数用于强制注入微服务名称，一般会固定在代码中，若不修改该参数为空
def EXPORT = "export username=commonnew_prod export password=QO0JFZCQX412T4koTZAf"
...
def Xmx = '1024M'
def Threads = '2'
def JAVA_OPTS = "-Xms${Xms} -Xmx2048M -XX:PermSize=${Xms} -XX:MaxNewSize=${Xms} -XX:MaxPermSize=${Xmx} -XX:ParallelGCThreads=${Threads}"//java启动参数，若有需要请自行添加
...
mavenJob("${COMPILE_JOB_NAME}") {
    ...
    preBuildSteps {
        shell("export JAVA_HOME=/usr/java/jdk1.8.0_45/jre; cd ./cloud-core; mvn clean install")
    }
    ...
    publishers {
        aliyunOSSPublisher {
          ...
          filesPath('oauth2-server/target/*.jar ')
...
pipelineJob("$PIPELINE_JOB_NAME") {
  ...
  definition {
    cpsScm {
      ...
      scriptPath("jenkins-pipelines/prodd/Insurance/Prod-insurance-ms-oauth-Pipeline.groovy")
    }
  }
}