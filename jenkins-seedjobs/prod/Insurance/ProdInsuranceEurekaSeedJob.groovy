...
def COMPILE_JOB_NAME = Folder+'/'+'Prod-Insurance-Eureka-Compile' //编译job名称，不可以使用中文
def DEPLOY_JOB_NAME1 = Folder+'/'+'Prod-Insurance-Eureka-Deploy1' //发布job名称，不可以使用中文
def DEPLOY_JOB_NAME2 = Folder+'/'+'Prod-Insurance-Eureka-Deploy2' //发布job名称，不可以使用中文
def PIPELINE_JOB_NAME = Folder+'/'+'Prod-Insurance-Eureka-Pipeline'
...
def ENV = 'prod'  //配置文件环境
def GROUP = 'qybx'  //所属组
def PRODUCTION = 'eureka'   //产品名称，该参数表示oss存储路径和部署服务器路径中微服务名
def WAR_NAME = 'eureka-server-1.0-SNAPSHOT.jar'  //WAR包名称
...
mavenJob("${COMPILE_JOB_NAME}") {
    ...
    publishers {
        aliyunOSSPublisher {
          ...
          filesPath('eureka-server/target/*jar')
...
pipelineJob("$PIPELINE_JOB_NAME") {
  ...
  definition {
    cpsScm {
      ...
      scriptPath("jenkins-pipelines/prodd/Insurance/Prod-insurance-eureka-Pipeline.groovy")
    }
  }
}