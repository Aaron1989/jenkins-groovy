...
def COMPILE_JOB_NAME = Folder+'/'+'Prod-Insurance-Link-Customer-Compile' //编译job名称，不可以使用中文
def DEPLOY_JOB_NAME1 = Folder+'/'+'Prod-Insurance-Link-Customer-Deploy1' //发布job名称，不可以使用中文
def DEPLOY_JOB_NAME2 = Folder+'/'+'Prod-Insurance-Link-Customer-Deploy2' //发布job名称，不可以使用中文
def PIPELINE_JOB_NAME = Folder+'/'+'Prod-Insurance-Link-Customer-Pipeline'
...
def ENV = 'prod'  //配置文件环境
def GROUP = 'qybx'  //所属组
def PRODUCTION = 'customer-link'   //产品名称，该参数表示oss存储路径和部署服务器路径中微服务名
def WAR_NAME = 'customer_link-0.0.1-SNAPSHOT.jar'  //WAR包名称
def OSS_BUCKET = 'deployment-package'  //oss存储bucket

def APP_OPTS = "--eureka.backup.zone= "   //此参数用于强制注入微服务名称，一般会固定在代码中，若不修改该参数为空
def EXPORT = "export mysql_business_name=qybxlink_prod export mysql_business_pass=mHX7pXRRx6zFMadeQ1aZ export redis_pass=dasDHkkJGU12U08R export mq_ak=LTAIU3Regn2lz7cE"
...
mavenJob("${COMPILE_JOB_NAME}") {
    ...
    publishers {
        aliyunOSSPublisher {
          ...
          filesPath('customer_link/target/*.jar')
...
pipelineJob("$PIPELINE_JOB_NAME") {
  ...
  definition {
    cpsScm {
      ...
      scriptPath("jenkins-pipelines/prodd/Insurance/Prod-insurance-link-customer-Pipeline.groovy")
    }
  }
}