...
def COMPILE_JOB_NAME = Folder+'/'+'Prod-Insurance-Link-CustomerCommon-Compile' //编译job名称，不可以使用中文
def DEPLOY_JOB_NAME1 = Folder+'/'+'Prod-Insurance-Link-CustomerCommon-Deploy1' //发布job名称，不可以使用中文
def DEPLOY_JOB_NAME2 = Folder+'/'+'Prod-Insurance-Link-CustomerCommon-Deploy2' //发布job名称，不可以使用中文
def PIPELINE_JOB_NAME = Folder+'/'+'Prod-Insurance-Link-Pipeline'
...
def ENV = 'prod'  //配置文件环境
def GROUP = 'qybx'  //所属组
def PRODUCTION = 'customer-common'   //产品名称，该参数表示oss存储路径和部署服务器路径中微服务名
def WAR_NAME = 'customer-common-1.0.0.jar'  //WAR包名称
...
mavenJob("${COMPILE_JOB_NAME}") {
    ...
    publishers {
        aliyunOSSPublisher {
          ...
          filesPath('customer-common/target/*.jar')
...
pipelineJob("$PIPELINE_JOB_NAME") {
  ...
  definition {
    cpsScm {
      ...
      scriptPath("jenkins-pipelines/prodd/Insurance/Prod-insurance-link-Pipeline.groovy")
    }
  }
}