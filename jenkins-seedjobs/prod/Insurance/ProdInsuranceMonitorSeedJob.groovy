...
def COMPILE_JOB_NAME = Folder+'/'+'Prod-Insurance-Monitor-Compile' //编译job名称，不可以使用中文
def DEPLOY_JOB_NAME1 = Folder+'/'+'Prod-Insurance-Monitor-Deploy1' //发布job名称，不可以使用中文
...
def GROUP = 'qybx'  //所属组
def PRODUCTION = 'insurance_monitor'   //产品名称，该参数表示oss存储路径和部署服务器路径中微服务名
def WAR_NAME = 'insurance_monitor-1.0-SNAPSHOT.jar'  //WAR包名称
...
pipelineJob("$PIPELINE_JOB_NAME") {
  ...
  definition {
    cpsScm {
      ...
      scriptPath("jenkins-pipelines/prodd/Insurance/Prod-insurance-monitor-Pipeline.groovy")
    }
  }
}