...
def COMPILE_JOB_NAME = Folder+'/'+'Prod-Insurance-Zuul-Router-Compile' //编译job名称，不可以使用中文
def DEPLOY_JOB_NAME1 = Folder+'/'+'Prod-Insurance-Zuul-Router-Deploy1' //发布job名称，不可以使用中文
def DEPLOY_JOB_NAME2 = Folder+'/'+'Prod-Insurance-Zuul-Router-Deploy2' //发布job名称，不可以使用中文
def PIPELINE_JOB_NAME = Folder+'/'+'Prod-Insurance-Zuul-Router-Pipeline'

def GITURL = 'http://h.quyiyuan.com/scm/in/insurance_cloud.git' //git仓库地址
def Git_AUTH = '6b5de867-3f96-4162-9384-4c81e0e7063c' //git仓库认证账号
def BRANCH = ''  //发布分支

def ENV = 'prod'  //配置文件环境
def GROUP = 'qybx'  //所属组
def PRODUCTION = 'zuul-router'   //产品名称，该参数表示oss存储路径和部署服务器路径中微服务名
def WAR_NAME = 'zuul-router-1.0-SNAPSHOT.jar'  //WAR包名称
def OSS_BUCKET = 'deployment-package'  //oss存储bucket

def APP_OPTS1 = "--eureka.backup.zone= "   //此参数用于强制注入微服务名称，一般会固定在代码中，若不修改该参数为空
def APP_OPTS2 = "--eureka.backup.zone= "   //此参数用于强制注入微服务名称，一般会固定在代码中，若不修改该参数为空
def EXPORT = ""
...
mavenJob("${COMPILE_JOB_NAME}") {
    ...
    publishers {
        aliyunOSSPublisher {
          ...
          filesPath('zuul-router/target/*.jar')
...
freeStyleJob("$DEPLOY_JOB_NAME1") {
    ...
    parameters {
        ...
        stringParam ('WAR_NAME',"$WAR_NAME")
        textParam ('JPS_PARAMETER',"{'PRODUCTION':'$PRODUCTION','GROUP':'$GROUP','APP_OPTS':'$APP_OPTS1','ENV':'$ENV','JAVA_OPTS':'$JAVA_OPTS','EUREKA_OPTS':'$EUREKA_OPTS',"+'"TAG":"$TAG","WAR_NAME":"$WAR_NAME","EXPORT":"$EXPORT"}')
...
freeStyleJob("$DEPLOY_JOB_NAME2") {
    ...
    parameters {
        ...
        stringParam ('OLD_VERSION',"")
        stringParam ('WAR_NAME',"$WAR_NAME")
        textParam ('JPS_PARAMETER',"{'PRODUCTION':'$PRODUCTION','GROUP':'$GROUP','APP_OPTS':'$APP_OPTS2','ENV':'$ENV','JAVA_OPTS':'$JAVA_OPTS','EUREKA_OPTS':'$EUREKA_OPTS',"+'"TAG":"$TAG","WAR_NAME":"$WAR_NAME","EXPORT":"$EXPORT"}')
        stringParam ('SHELL_NAME',"$SHELL_NAME")
...
pipelineJob("$PIPELINE_JOB_NAME") {
  ...
  definition {
    cpsScm {
      ...
      scriptPath("jenkins-pipelines/prodd/Insurance/Prod-insurance-zuul-router-Pipeline.groovy")
    }
  }
}