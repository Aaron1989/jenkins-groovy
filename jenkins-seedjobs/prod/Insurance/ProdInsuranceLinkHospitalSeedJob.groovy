...
def COMPILE_JOB_NAME = Folder+'/'+'Prod-Insurance-Link-Hospital-Compile' //编译job名称，不可以使用中文
def DEPLOY_JOB_NAME1 = Folder+'/'+'Prod-Insurance-Link-Hospital-Deploy1' //发布job名称，不可以使用中文
def DEPLOY_JOB_NAME2 = Folder+'/'+'Prod-Insurance-Link-Hospital-Deploy2' //发布job名称，不可以使用中文
def PIPELINE_JOB_NAME = Folder+'/'+'Prod-Insurance-Link-Hospital-Pipeline'

def GITURL = 'http://h.quyiyuan.com/scm/in/insurance_link.git' //git仓库地址
def Git_AUTH = '6b5de867-3f96-4162-9384-4c81e0e7063c' //git仓库认证账号
def BRANCH = ''  //发布分支

def ENV = 'prod'  //配置文件环境
def GROUP = 'qybx'  //所属组
def PRODUCTION = 'hospital-link'   //产品名称，该参数表示oss存储路径和部署服务器路径中微服务名
def WAR_NAME = 'hospital_link-0.0.1-SNAPSHOT.jar'  //WAR包名称
def OSS_BUCKET = 'deployment-package'  //oss存储bucket

def APP_OPTS = "--eureka.backup.zone= "    //此参数用于强制注入微服务名称，一般会固定在代码中，若不修改该参数为空
def EXPORT = "export mysql_business_name=qybxlink_prod export mysql_business_pass=mHX7pXRRx6zFMadeQ1aZ export mysql_common_name=qybxcommon_prod export mysql_common_pass=LMVmK30UbyqgzckYtvyr export redis_pass=dasDHkkJGU12U08R export mq_ak=LTAIU3Regn2lz7cE"
...
mavenJob("${COMPILE_JOB_NAME}") {
    ...
    preBuildSteps {
        shell("export JAVA_HOME=/usr/java/jdk1.8.0_45/jre; cd ./link-core; mvn clean install -Dmaven.test.skip=true")
    }
    scm {
            git {
                remote {
    ...
    publishers {
        aliyunOSSPublisher {
          ...
          filesPath('hospital_link/target/*.jar')
...
pipelineJob("$PIPELINE_JOB_NAME") {
  ...
  definition {
    cpsScm {
      ...
      scriptPath("jenkins-pipelines/prodd/Insurance/Prod-insurance-link-hospital-Pipeline.groovy")
    }
  }
}