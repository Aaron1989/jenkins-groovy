...
def COMPILE_JOB_NAME = Folder+'/'+'Prod-Hms-Ms-Aid-Compile' //编译job名称，不可以使用中文
def DEPLOY_JOB_NAME1 = Folder+'/'+'Prod-Hms-Ms-Aid-Deploy1' //发布job名称，不可以使用中文
def DEPLOY_JOB_NAME2 = Folder+'/'+'Prod-Hms-Ms-Aid-Deploy2' //发布job名称，不可以使用中文
def PIPELINE_JOB_NAME1 =  Folder+'/'+'Prod-Hms-Ms-Aid-Pipeline'
...
def GROUP = 'hms'  //所属组
def PRODUCTION = 'ms-aid'   //产品名称，该参数表示oss存储路径和部署服务器路径中微服务名
def WAR_NAME = '*.war'  //WAR包名称

def EXPORT =''
def APP_NAME = 'ms-aid'  //微服务名称,一般固定在代码里，若不修改则为空
...
/*freeStyleJob("$OSS_JOB_NAME"){
    ...
    preBuildSteps {
        shell(
                "npm run build\n" +
                "python /data/osscmd uploadfromdir workspace/dist/ oss://qy-hms-jijiu --host=oss-cn-qingdao-internal.aliyuncs.com  --check_md5=true"
        )
    }
}*/
...
pipelineJob("$PIPELINE_JOB_NAME1") {
    ...
    definition {
        cpsScm {
            ...
            scriptPath("jenkins-pipelines/prodd/Hms/Prod-Hms-MsAid-Pipeline.groovy")
        }
    }
}