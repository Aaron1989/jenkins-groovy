...
def COMPILE_JOB_NAME = Folder+'/'+'Prod-Hms-MSHMS-RDS-Compile' //编译job名称，不可以使用中文
def DEPLOY_JOB_NAME = Folder+'/'+'Prod-Hms-MSHMS-RDS-Deploy' //发布job名称，不可以使用中文
def PIPELINE_JOB_NAME =  Folder+'/'+'Prod-Hms-MSHms-RDS-Pipeline'
def ECS_IP = [' ']
...
pipelineJob("$PIPELINE_JOB_NAME") {
    ...
    definition {
        cpsScm {
            ...
            scriptPath("jenkins-pipelines/prodd/Hms/Prod-Hms-MsHms-Rds-Pipeline.groovy")
        }
    }
}
