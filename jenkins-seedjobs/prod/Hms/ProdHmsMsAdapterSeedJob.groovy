...
def PIPELINE_JOB_NAME1 =  Folder+'/'+'Prod-Hms-Ms-Adapter-Pipeline'
def ECS_IP = [' ']
...
def WAR_NAME = '*.jar'  //WAR包名称
def EXPORT =''
def APP_NAME = 'ms-adapter'  //微服务名称,一般固定在代码里，若不修改则为空
...
pipelineJob("$PIPELINE_JOB_NAME1") {
    ...
    definition {
        cpsScm {
            ...
            scriptPath("jenkins-pipelines/prodd/Hms/Prod-Hms-MsAdapter-Pipeline.groovy")
        }
    }
}