...
def COMPILE_JOB_NAME = Folder+'/'+'Prod-Hms-MS-DES-Compile' //编译job名称，不可以使用中文
def DEPLOY_JOB_NAME1 = Folder+'/'+'Prod-Hms-MS-DES-Deploy1' //发布job名称，不可以使用中文
def DEPLOY_JOB_NAME2 = Folder+'/'+'Prod-Hms-MS-DES-Deploy2' //发布job名称，不可以使用中文
def PIPELINE_JOB_NAME1 =  Folder+'/'+'Prod-Hms-MS-DES-Pipeline'
//def OSS_JOB_NAME = Folder+'/'+'Prod-Hms-Ms-Des-OSS'
def ECS_IP = [' ',' ']

def GITURL = 'http://h.quyiyuan.com/scm/ysapp/ms-des.git' //git仓库地址
...
def GROUP = 'hms'  //所属组
def PRODUCTION = 'ms-des'   //产品名称，该参数表示oss存储路径和部署服务器路径中微服务名
def WAR_NAME = 'ms-des-*.war'  //WAR包名称

def EXPORT =''
def APP_NAME = 'ms-des'  //微服务名称,一般固定在代码里，若不修改则为空
...
freeStyleJob("$OSS_JOB_NAME"){
    ...
    preBuildSteps {
        shell(
                "cd /mnt/jenkins/jobs/Prod-Hms-Ms-Des-OSS/workspace\n" +
                "npm install\n" +
                "npm run build\n"+
                "cd /data\n"+
                "python osscmd uploadfromdir /workspace/dist/ oss://qy-hms-zhibo-prod --host=oss-cn-qingdao-internal.aliyuncs.com  --check_md5=true"
        )
    }
    scm {
        git {
...
pipelineJob("$PIPELINE_JOB_NAME1") {
    ...
    definition {
        cpsScm {
            ...
            scriptPath("jenkins-pipelines/prodd/Hms/Prod-Hms-MsDes-Pipeline.groovy")
        }
    }
}