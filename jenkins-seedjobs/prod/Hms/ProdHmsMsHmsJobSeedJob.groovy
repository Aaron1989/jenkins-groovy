...
def COMPILE_JOB_NAME = Folder+'/'+'Prod-Hms-MS-HMS-Job-Compile' //编译job名称，不可以使用中文
def DEPLOY_JOB_NAME = Folder+'/'+'Prod-Hms-MS-HMS-Job-Deploy' //发布job名称，不可以使用中文
def PIPELINE_JOB_NAME1 =  Folder+'/'+'Prod-Hms-MS-HMS-Job-Pipeline'
def ECS_IP = [' ']

def GITURL = 'http://h.quyiyuan.com/scm/ysapp/ms-hms-job.git' //git仓库地址
...
def GROUP = 'hms'  //所属组
def PRODUCTION = 'ms-hms-job'   //产品名称，该参数表示oss存储路径和部署服务器路径中微服务名
def WAR_NAME = 'ms-hms-job-1.0.0.war'  //WAR包名称

def EXPORT =''
def APP_NAME = 'ms-hms-job'  //微服务名称,一般固定在代码里，若不修改则为空
...
pipelineJob("$PIPELINE_JOB_NAME1") {
    ...
    definition {
        cpsScm {
            ...
            scriptPath("jenkins-pipelines/prodd/Hms/Prod-Hms-MsHms-Job-Pipeline.groovy")
        }
    }
}