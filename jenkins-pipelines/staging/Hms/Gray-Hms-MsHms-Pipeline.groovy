def COMPILE_JOB_NAME = 'Gray-Hms-MS-HMS-Compile'
def DEPLOY_JOB_NAME = 'Gray-Hms-MS-HMS-Deploy'
...
node {
    stage('Compile') {
        build job: "$COMPILE_JOB_NAME", parameters: [string(name: 'TAG', value:env.TAG), string(name: 'BRANCH', value: env.BRANCH), string(name: 'WAR_NAME', value: WAR_NAME)]
    }
    /*stage ('input'){
        def userInput = input(
