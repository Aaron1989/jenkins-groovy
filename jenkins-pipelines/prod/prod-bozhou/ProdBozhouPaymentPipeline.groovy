...
def DEPLOY_JOB_NAME1 = 'Prod-Bozhou-bzxnh-payment-deploy1' //发布job名称，不可以使用中文
def DEPLOY_JOB_NAME2 = 'Prod-Bozhou-bzxnh-payment-deploy2' //发布job名称，不可以使用中文
def GROUP = 'web'
...
node ('base'){
    ...
    stage('deploy notification'){
        ...
        stage('Deploy1') {
        input 'ok?'
        JobResult = build job : "$DEPLOY_JOB_NAME1",parameters :[string(name: 'TAG', value:env.TAG), string(name: 'WAR_NAME', value:env.WAR_NAME)]
        pipelineStatus = JobResult.result
        echo 'deployment result is: + pipelineStatus'
        }
        stage('Deploy2') {
        input 'ok?'
        JobResult = build job : "$DEPLOY_JOB_NAME2",parameters :[string(name: 'TAG', value:env.TAG), string(name: 'WAR_NAME', value:env.WAR_NAME)]
        pipelineStatus = JobResult.result
        echo 'deployment result is: + pipelineStatus'
        }
