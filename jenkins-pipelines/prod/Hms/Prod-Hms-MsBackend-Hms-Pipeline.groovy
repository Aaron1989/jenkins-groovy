def COMPILE_JOB_NAME = 'Prod-Hms-BackEnd-HMS-Compile'
def DEPLOY_JOB_NAME = 'Prod-Hms-BackEnd-HMS-Deploy'
...
node {
    ...
    stage('Deploy') {
        ...
        build job: "$DEPLOY_JOB_NAME",parameters :[string(name: 'WAR_NAME',value: env.WAR_NAME),string(name: 'TAG', value:env.TAG), string(name: 'OLD_VERSION', value:env.OLD_VERSION), password(name: 'JPS_PWD', value:env.JPS_PWD)]
    }
}
node('base'){
