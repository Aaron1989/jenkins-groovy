...
def jpsApi = 'http://100.115.142.129:8002'
def apolloUrl = 'http://10.31.87.55:8070/configs/'
def env = 'dev'
def nameSpaces = ['prod', 'prod_cloud', 'gray_cloud']
def applicationName = 'ms-oldhms-rds'
def compileJobName = 'Prod-Hms-MSHMS-RDS-Compile1'
    ...
    stage('Compile') {
        ...
        build job: compileJobName, parameters: [string(name: 'PREFIX', value: prefix), string(name: 'GIT_URL',
                value: prodInfo['git_url']), string(name: 'VERSION', value: prodInfo['tag']), string(name: 'BRANCH', value: prodInfo['branch']),
                   string(name: 'WAR_NAME', value: prodInfo['war_name']), string(name: 'DESCRIPTION', value: description)]
...
//                version + '", "rollbackVersion": "' + old_version + '", "JIRARefusedReason": "' + JIRARefusedReason + '"}'
        build job: 'Prod-Public/Prod-Public-Jira-Confirm', parameters: [string(name: 'VERSION', value: prodInfo['tag']),
                string(name: 'OLD_VERSION', value: prodInfo['old_version']), string(name: 'APP_NAME', value: prodInfo['service']),
    ...
    stage('Merge') {
        build job: 'Prod-Public/Prod-Public-Merge-Job', parameters: [string(name: 'VERSION', value: prodInfo['tag']),
                 string(name: 'REPOSITORY_URL', value: prodInfo['git_url']), string(name: 'BRANCH', value: prodInfo['branch']),
