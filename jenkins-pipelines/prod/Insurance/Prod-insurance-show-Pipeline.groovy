def COMPILE_JOB_NAME = 'Prod-Insurance-Show-Compile' //编译job名称，不可以使用中文
def DEPLOY_JOB_NAME = 'Prod-Insurance-Show-Deploy' //发布job名称，不可以使用中文
...
def WAR_NAME = 'insurance_demonstrate_platform-1.0-SNAPSHOT.jar'
properties ([
    parameters ([
...
def TAG = env.TAG
def OLD_VERSION = env.OLD_VERSION
def JPS_PWD = env.JPS_PWD
def BRANCH = env.BRANCH
def APP_NAME = env.APP_NAME
def Description = env.Description
def JiraID = env.JiraID
...
node {
    stage('deploy notification'){
        ...
        build job: deploy_notification_job, parameters: [string(name: 'TAG', value: env.TAG), string(name: 'JiraID', value: env.JiraID), text(name: 'Description', value: env.description), string(name: 'group', value: GROUP), string(name: 'app_name', value: env.APP_NAME), string(name: 'stage', value: 'start')]
