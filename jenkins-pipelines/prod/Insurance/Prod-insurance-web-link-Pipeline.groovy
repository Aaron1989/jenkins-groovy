def COMPILE_JOB_NAME = 'Prod-Insurance-Web-Link-Compile' //编译job名称，不可以使用中文
def DEPLOY_JOB_NAME1 = 'Prod-Insurance-Web-Link-Deploy1' //发布job名称，不可以使用中文
def DEPLOY_JOB_NAME2 = 'Prod-Insurance-Web-Link-Deploy2' //发布job名称，不可以使用中文
def IP_LIST = [' ',' ']
...
def WAR_NAME = 'insurance-web-link-1.0-SNAPSHOT.jar'
...
node('deploy'){
    for (def ip: IP_LIST) {
        stage('Deploy' + ip) {
            input 'ok?'
            JobResult = build job: "$DEPLOY_JOB_NAME" + ip, parameters: [string(name: 'TAG', value: env.TAG), string(name: "OLD_VERSION", value: env.OLD_VERSION), string(name: 'WAR_NAME', value: env.WAR_NAME), password(name: 'JPS_PWD', value: env.JPS_PWD)]
